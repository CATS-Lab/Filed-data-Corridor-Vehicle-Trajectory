...
for veh in tqdm(veh_list, desc='Processing vehicles'):
    ...
    if edge_start != edge_end:
        Veh_info = Veh_info.append(
            {'id': veh, 'length': round(l, 2), 'start_time': round(d['t_sec'].min(), 1), 'trip_time': round(t, 1),
             'edge_start': edge_start, 'edge_end': edge_end}, ignore_index=True)
