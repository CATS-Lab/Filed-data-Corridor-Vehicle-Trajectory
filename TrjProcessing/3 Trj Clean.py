...
def CleanSharpAngle(x_list, y_list, angle_threshold):
    ...
    for i in range(1, len(x_list) - 2):
        ...
        if angle < angle_threshold:
            x_new[i], y_new[i] = np.mean([x_list[i - 1], x_list[i + 1], x_list[i + 2]]), np.mean(
                [y_list[i - 1], y_list[i + 1], y_list[i + 2]])
...
def MovingAverage(input):
    ...
    a, b, c, d = 4, 3, 2, 1
    w = np.array([4, 3, 2, 1])
    output[size - 2] = (a * input[size - 1] + b * input[size - 2] + c * input[size - 3] + d * input[size - 4]) / (
                a + b + c + d)
    a, b, c, d = 3, 2, 1, -1
    w = np.array([3, 2, 1, -1])
    output[size - 1] = (a * input[size - 1] + b * input[size - 2] + c * input[size - 3] + d * input[size - 4]) / (
                a + b + c + d)
    return output
